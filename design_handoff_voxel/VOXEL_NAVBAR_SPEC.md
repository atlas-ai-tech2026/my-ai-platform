...
function CreditButton({ credits = 40000, total = 50000, renewsOn = 'May 15, 2026', plan = 'PRO' }) {
  ...
  // Progress ring geometry — sized to sit OUTSIDE a 40×40 inner disc
  const R = 22;                         // ring radius
  const CIRC = 2 * Math.PI * R;         // circumference
  // Ring shrinks as credits are used (full when remaining = total)
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      {/* ─── Circle button with progress ring ───
          TWO separate <svg>s so the 360° track always shows a clean full ring
          and the depleting arc rides on top. Inner disc has NO border — the
          ring IS the border. This prevents the "broken edge" artifact. */}
      <button
        onClick={() => setOpen(v => !v)}
        aria-label="Credits"
        style={{
      ...
      >
        {/* Full 360° track — NOT rotated, always complete */}
        <svg
        ...
        {/* Depleting active arc — rotated so it starts at 12 o'clock */}
        <svg
          width="48" height="48" viewBox="0 0 48 48"
          style={{
        ...
        {/* Circle body — inset 4px, NO border (the ring IS the border) */}
        <div style={{
          position: 'absolute', top: 4, left: 4, width: 40, height: 40,
          borderRadius: '50%',
          background: open
            ? `radial-gradient(circle at 50% 45%, rgba(224,30,30,0.4), rgba(224,30,30,0.15))`
            : `radial-gradient(circle at 50% 45%, rgba(224,30,30,0.28), rgba(224,30,30,0.1))`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: open
            ? `0 0 18px rgba(224,30,30,0.5)`
            : `0 0 12px rgba(224,30,30,0.25)`,
