...
export const SEED = [
  ...
  { ref: '80', owner: 'claude', status: 'pending', priority: 8,
    ...
    detail: '── RECOMMENDATION: NO, NOT NOW — and the reasons are in order of how decisive they are ── '
      ...
      + '── WHAT WOULD BE PROPORTIONATE LATER ── a WARM STANDBY, not active-active: a written, REHEARSED procedure to redeploy on a second provider from the existing backups within a few hours, tested once a quarter the way #34 tested the restore. That is achievable, cheap, and fails safely. Revisit when a workshop cancellation would cost more than the standby costs to run, or when the DNS is ours and a real failover switch exists.' },
