...
export const SEED = [
  ...
  { ref: '101', owner: 'claude', status: 'done', priority: 1,
    title: 'Every Kling choice generates from its kie twin — built on the branch, nothing deployed until the owner says go',
    why: 'Owner, 2026-08-25 and again 2026-09-03: "If the user chooses Kling 3.0, generate from kie\'s Kling 3.0. If Kling Omni, from kie\'s Kling Omni. If Motion Control, from kie\'s Motion Control. Do not generate anything from the API of FAL." Five Kling models were still on FAL, one of them under a name kie has never had — and the multi-shot complaint (the original #101) most likely came from the Omni picker entry that ran on FAL with no shot control at all.',
    detail: 'CLOSED 2026-09-05 at the owner\'s instruction. The branch landed: commit 17cbb3d (every Kling model generates from its kie twin) is on production, verified as an ancestor of origin/main. The title\'s "nothing deployed until the owner says go" is no longer true — he said go, and it went. ── THE PROBLEM, EXACTLY ── The picker listed 9 Kling models. Four ran on kie (3.0, 3.0 Turbo, 2.6, and Motion Control was NOT among them). Five ran on FAL: Kling 3.0 Omni (FAL kling v3 pro — a different product from kie\'s Kling O3), Kling 2.5 (FAL v1.5 pro), Kling 2.1 (FAL v2.1 standard), "Kling O1" (FAL\'s Kling 1.6 renamed — kie has no O1), both Motion Controls and both Edit models (FAL). No error ever showed: FAL answered, the charge landed, history said "Kling". '
      + '── FIXED ON THE BRANCH 2026-09-03 ── (1) Kling 3.0 Omni → kie Kling O3: ids kling-3.0-omni/text-to-video (read from kie\'s docs) and kling-3.0-omni/image-to-video (kie\'s naming; if kie rejects it the route retries ONCE on the t2v id with the frames and logs "[KIE-VIDEO] model id fallback" — that log line means the id must be corrected). multi_shots:false unless Multi Shot is on; 4K not requested (no 4K price on file). (2) Kling 2.5 → kie 2.5 Turbo Pro (kling/v2-5-turbo-image-to-video-pro, the id kie publishes; t2v sibling id). (3) Kling 2.1 → kie 2.1 Standard (kling/v2-1-standard) — kie\'s 2.1 is IMAGE-TO-VIDEO ONLY, so a text-only request is refused before charging with a named reason; the card says Start Frame. (4) Kling O1 retired from picker, router and price tables. (5) Both Motion Controls → kling-3.0/motion-control and kling-2.6/motion-control (input_urls, video_urls, mode std/pro, character_orientation on 3.0). (6) Edit Video paused — see #102. (7) A test, kling-on-kie.test.js, fails the build if any Kling-labelled entry ever points at FAL again, if the Motion route touches fal.queue, or if the picker lists a Kling model not routed to kie. '
      + '── PRICES, BY THE CALCULATOR (cost ÷ 0.6 ÷ $0.063333, up to the next half credit) ── Motion Control was FLAT per clip (8/11 and 6/8) while kie bills PER SECOND of the reference clip (3–30 s): a 30-second Kling 3.0 clip at flat 11 would sell $0.70 of video that costs $1.80. Now per second. Kling 3.0 Motion Control: kie\'s OWN line, read by the owner from kie\'s page on 2026-09-03 — 20 credits/s ($0.10) at 720p, 27 credits/s ($0.135) at 1080p → 3 cr/s (47.4%) and 4 cr/s (46.7%). The figures quoted before that page was read (9 / 12 credits/s) were HALF the real cost — had they been priced, every clip would have sold below cost; this is why a price is never set from a quote. Kling Motion Control (2.6): kie\'s OWN line, read by the owner the same day — 11 credits/s ($0.055) at 720p, 18 credits/s ($0.09) at 1080p → 1.5 cr/s (42.1%) and 2.5 cr/s (43.2%). Kling 2.1: 1 cr/s (kie\'s own line: 25 credits = $0.125 per 5 s; was 2 cr/s against a FAL basis). Kling 2.5: unchanged 1.2 cr/s (kie $0.042/s, 44.7%). Kling 3.0 Omni: unchanged 4 cr/s until kie\'s O3 price line is read. '
      + '── THE BILLING NUMBER IS READ FROM THE FILE ── the browser sends the reference clip\'s length, but the server reads the MP4/MOV header itself (media-probe.js, Range requests, ~3 small reads) and bills THAT; the browser\'s number is only a cross-check, used solely when the file is unreadable (WebM) and then logged. A price resting on a client-supplied number is the C1 hole. '
      + '── KEPT FROM 2026-08-25 ── the continuity instruction in the provider-facing prompt for every Kling image-to-video request with Multi Shot off (video-prompt.js, 8 tests). '
      + '── FOR THE OWNER, BEFORE PRODUCTION ── (a) both Motion Control price lines are in — nothing owed here; (b) confirm Edit Video may stay paused (#102) rather than run on FAL; (c) then dev → one real generation on each of Omni (image), 2.5, 2.1 (image), Motion Control 3.0 — the [KIE-VIDEO] / [MOTION-CONTROL] payload lines and the absence of "model id fallback" are the proof — → production.' },
  { ref: '105', owner: 'claude', status: 'done', priority: 1,
    title: 'Workshop credit report — every email, the reason, the credits, as a PDF',
    why: 'Owner, 2026-09-03: "The workshop a few days ago — we gave every student $25, 395 credits, with the reason SPA4 in the CRM. I need a PDF report: how many emails, the reason, how many credits. I need to know everything, in a professional way."',
    detail: 'CLOSED 2026-09-05 at the owner\'s instruction. Commits 3a77f05, 56fa60f and cced814 are on production. WHERE THE ANSWER LIVES: every credit ever added is a ledger row (credits_history) with the email, the amount, the date, the admin who did it and the reason they typed. The Logs tab already filters those rows by reason text and by type, but shows 50 at a time and exports only the page on screen. '
      + 'BUILT ON THE BRANCH (2026-09-03): a Report (PDF) button and a Report (Excel) button on the Logs tab. They take the filters exactly as set on the screen — for the workshop: Status = grant, details = SPA4 — fetch EVERY matching row from a new admin route (/api/admin/reports/credits, same filter builder as the Logs page so the two can never disagree), and open an A4 report in its own tab: title, the filters in words, when it was generated, four totals (people, entries, credits, dollar value at $19/300 per credit), the CHECKS a bookkeeper does by hand (any email that received it twice; any entry whose amount differs from the usual one, e.g. not 395), then the full table (#, email, date, credits, type, added by, reason) with a totals row. Save as PDF is the browser\'s own print dialog — no new software. The Excel file carries the same rows and totals. Every value is escaped; a typed reason can never run as code. Numbers tested (creditReport.test.js); route pinned (credit-report.test.js). '
      + 'HONESTY: the report reads the live ledger at the moment it is opened; the "usual amount" is the most common amount in the result, not a stored rule. Rows are capped at 5,000 and the report says so if the cap bit. '
      + 'OWED after the owner confirms it reads right: ⓘ tooltips are on both buttons; a Knowledge Base entry in Arabic and English with pictures (how to run the workshop report). '
      + 'TO PRODUCTION: this is independent of the Kling / Seedance work on dev, so it can go to production on its own — the owner decides whether it ships alone or with the rest.' },
  { ref: '104', owner: 'claude', status: 'done', priority: 1,
    title: 'Seedance 2.5 refused every job that edits an attached video — the output must follow the video',
    why: 'Owner, 2026-09-03, with kie\'s message: "Seedance identified your task as video editing based on your prompt … ratio must be adaptive … duration must be -1 … the video must be 4 to 30 seconds." A customer attached a video and an audio clip and got that back instead of a video.',
    detail: 'CLOSED 2026-09-05 at the owner\'s instruction. Commits 9fc27ad and 55d15da are on production. WHAT WAS HAPPENING: Seedance reads the prompt and, when it decides the job EDITS the attached video, the result takes the input video\'s length and shape — so it refuses any fixed length or ratio. Voxel always sent a fixed length: "Auto" on the picker was silently 5 seconds, and a chosen ratio went through as chosen. Every editing-type job with a video attached was refused by kie; the route refunded and showed kie\'s reason. Identical on production and the branch; not related to the Kling work. '
      + 'FIXED ON THE BRANCH (2026-09-03): with a reference video attached the request sends duration -1 and ratio adaptive (kie: "the model picks" — valid for any Seedance 2.x job, required for editing); the panel greys the two pickers and says "Length and ratio follow your reference video". THE CHARGE is per second of the LONGEST reference video, read from the FILE on the server (media-probe.js — the same reader Motion Control uses); the browser\'s reading rides along only as a cross-check and is logged when it disagrees. A video shorter than 4 s or longer than the model\'s ceiling (30 s on 2.5, 15 s on the 2.0 family) is refused with a plain sentence BEFORE any charge. Reference videos and audio are now re-hosted for kie like every other input (kie cannot read a data: URI). The full kie payload is logged. Helper seedance-reference.js (7 tests) + source test seedance-reference-video.test.js. '
      + 'FOUND ON THE OWNER\'S DEV TEST (2026-09-03, first try): the price said 19 s and the card badge said 0:05 — the badge shows the stored duration and the page had stored the picker\'s "auto", which the badge renders as 5. The route now returns the seconds it billed and the page labels the card with them. Fixed the same hour. '
      + 'STILL TRUE, NOT CHANGED: "Auto" duration WITHOUT a reference video still means 5 seconds — an honesty gap in the picker label, small, to decide separately. '
      + 'FOR THE OWNER ON DEV: one Seedance 2.5 job with a video + audio attached and an editing prompt ("change the …"): the [SEEDANCE] [KIE] payload line shows duration -1 / aspect_ratio adaptive, the charge equals rate × the video\'s seconds, and a video comes back.' },
  { ref: '102', owner: 'claude', status: 'pending', priority: 2,
    title: 'Edit Video is paused: its kie twin (Kling O3 video reference) needs one page read',
    why: 'Both Edit models ran on FAL. The owner\'s rule is that no Kling generation goes to FAL; kie has no Kling O1 at all, and Kling 3.0 Omni Edit\'s kie twin is Kling O3\'s video-reference mode, whose request field names (the reference video, the reference images, keep-sound) have not been read from kie\'s page — guessing them would produce silently wrong videos, since kie ignores unknown fields rather than rejecting them.',
    detail: 'STATE ON THE BRANCH: the Edit tab is off the Video page nav (VideoTopTabs.jsx); POST /api/edit-video-omni answers 503 with the reason and takes no credits; Kling O1 Video Edit is retired everywhere; Kling 3.0 Omni Edit keeps its 10-credit flat price. '
      + 'WHAT UNBLOCKS IT IN A MINUTE: the owner opens docs.kie.ai → Kling → the Kling 3.0 Omni page that takes a VIDEO (titled Video to Video / Reference to Video / Transformation) and sends the model id line, the input field list, and the price line — a screenshot is fine. Then: EDIT_VIDEO_MODELS gets the id, the route builds { model, input } for kie jobs exactly like Motion Control, the tab comes back, and the price is re-derived by the calculator from kie\'s line. '
      + 'ALTERNATIVE THE OWNER CAN CHOOSE: keep Edit Video on FAL until then. That contradicts the stated rule, so it was not done without asking.' },
  { ref: '103', owner: 'claude', status: 'pending', priority: 8,
    title: 'Five non-Kling models quietly run FAL\'s Kling for image-to-video — decide',
    why: 'Found while moving Kling to kie: Seedance 1, LTX 2, PixVerse 5, Vidu Q3 and Vidu Q2 have text-to-video on their own FAL endpoints, but their IMAGE-to-video entries all point at fal-ai/kling-video/v3/pro/image-to-video. A customer who picks PixVerse 5 and adds a start frame receives a Kling video and pays the PixVerse price. That is the same "the label is not the model" fault as the Kling ones, wearing other names.',
    detail: 'NOT changed in the Kling migration — these are not Kling models by label and none of the five exists on kie, so the rule "every Kling choice calls its kie twin" does not decide them. The list is pinned in kling-on-kie.test.js so it cannot grow unnoticed. '
      + 'OPTIONS: (a) remove the start-frame option for these five (honest: text-to-video only, on their own endpoint); (b) point each at its own FAL image-to-video endpoint where one exists (LTX, PixVerse, Vidu all have i2v on FAL — ids to be verified) and price by the calculator; (c) retire the five (they are the least used). Owner decides; (a) is the smallest honest change.' },
  { ref: '93', owner: 'claude', status: 'in_progress', priority: 3,
    title: 'Gemini Omni — references with frames now; Omni 1.1 Flash waiting on two facts from kie',
    why: 'Owner, 2026-08-25: Google Omni must take references AND start frame AND end frame, and kie has a new Google Omni 1.1 — add it. Half was the Seedance disease again: the backend accepted image_urls for Omni since it was wired (kie\'s 7-image budget), the card even says "with reference images" — and only the two frame slots ever fed it.',
    detail: 'PART 1 — BUILT, ON THE BRANCH: the generic panel gains a reference-image strip for capacity-flagged models (Omni: 7 total, SHARED with the frames and shown as a shared count, so the budget is honest). The route accepts reference_urls, re-hosts them exactly like frames, and the Omni mapping pools frames-first + refs, sliced at 7 — frames first so "@Image1" in a prompt keeps meaning the start frame. No price touched: Omni\'s per-(resolution,duration) table stands. '
      + 'PART 2 — GEMINI OMNI 1.1 FLASH, NOT WIRED YET, DELIBERATELY. kie\'s page confirms it exists (first/last-frame control, video references, 360p drafting, up to 4K; kie.ai/gemini-omni-1-1-flash) — but TWO facts must come from kie\'s own page, not from aggregators: (a) the EXACT model-id string their Jobs API takes, and (b) KIE\'S OWN price line. Both lessons already paid for: #76 shipped three wrong guesses about kie protocol details, and Seedance was nearly priced 33-50% high off fal\'s numbers. Outside sources quote Google\'s token pricing ($0.03/s 360p · $0.10/s 720p · $0.15/s 1080p · $0.30/s 4K) — that is GOOGLE\'S rate, not necessarily kie\'s. '
      + 'WHAT UNBLOCKS IT IN SIXTY SECONDS: the owner opens kie\'s Gemini Omni 1.1 Flash docs/playground and sends (screenshot is fine) the model id and the price/credit line. Then: model entry + card, the geminiOmni style extended for its first/last-frame fields, credits at the standard 40% formula presented for approval, dev, production.' },
  ...
  { ref: '81', owner: 'claude', status: 'done', priority: 1,
    ...
    detail: 'DECISIONS, all the owner\'s, 2026-08-25: (1) thirty days per addition, counted from its own addition date; (2) applies RETROACTIVELY to credits already in accounts, dated from the ledger; (3) every account locked by the old model gets access back. '
      + '── BUILT; MERGED TO DEV 2026-08-25 (e2225a3); TO PRODUCTION IN PR #78 — THE IN-APP CHECKS STILL UNVERIFIED ── Every credit addition (manual grant, promo, gift card, bulk batch) becomes a dated lot; a promo code\'s access_days now bounds its CREDITS\' life instead of extending the account lockout; spends drain soonest-expiring first inside chargeCredits, isolated behind a savepoint so a lots bug can never cost a generation; refunds land in the newest live lot or a fresh 30-day one. Existing balances are dated from the ledger at boot (newest-first, because spending drains oldest-first); balance the ledger cannot date is labelled and given the full 30 days from that day, never expired on a guess. An hourly sweep removes what passed its date — credits only, capped at the balance, one ledger row per removal naming the addition dates, safe under two instances because the claim is the SQL. '
      + '── NOTHING AUTOMATIC WRITES users.expires_at ANY MORE ── the manual-grant 30-day stamp, the promo GREATEST-extension and the bulk-create date are all gone; a wiring test counts the writers and fails if a new one appears. '
      + '── AND SINCE LATER THE SAME DAY, NO DOOR READS THE DATE AT ALL ── the original plan kept the sign-in checks until Activate cleared the stored dates (build before delete). That gate locked real workshop attendees out THREE TIMES in one day, because deployed code does not delete data and the press kept not happening. Removed: password door, social door, and the per-request middleware — the "Account has expired" message is extinct in code and a test keeps it extinct. The bulk close-access switch is unmounted from the CRM for the same reason a dead control must not look alive. '
  ...
  { ref: '76', owner: 'claude', status: 'done', priority: 1,
    ...
    detail: 'VERIFIED 2026-08-23, not guessed. Dev logs show [EDIT-AGENT] ❌ Forbidden twice. Ruled out from FAL\'s own public OpenAPI: google/gemini-flash-1.5 IS still a valid any-llm model, and the input shape (prompt required, model and system_prompt accepted) is correct. So the request is well-formed and the refusal is about the KEY or the ACCOUNT — consistent with FAL no longer being funded or used. '
      ...
      + '── ANSWERED AND BUILT 2026-08-24: kie HAS TEXT MODELS, so NO new key is needed ── I had framed the choice as revive FAL vs buy a Gemini/OpenAI key. Both wrong. kie.ai\'s public catalogue lists 27 CHAT models — gemini-3-7-flash, GPT-5.6, Claude Sonnet 5, Grok 4.6 — on an OpenAI-compatible /api/v1/chat/completions. The platform already pays for kie and KIE_KEY is already set on both apps (checked in the DO spec). No new account, no new key, no new dependency. '
      + 'THE ENHANCE BUTTONS WERE BROKEN TOO, and that is now confirmed rather than suspected: /api/enhance-prompt made the identical any-llm call with the identical model, copy-pasted. Same 403, same silence. A LIVE CUSTOMER FEATURE on voxel-ai.ai, dead, unreported — the owner happened to press the agent and not the red bolt. '
      + 'BUILT: server/src/llm.js, one place to ask a text model. kie by default; FAL still reachable via LLM_PROVIDER=fal so a revived key keeps working (build before you delete); LLM_MODEL swaps the model with no deploy. Both routes share it, so they cannot drift apart again. requireFalKey on those two routes became requireLlm — the old gate asked whether FAL had a key, which would wave a request through to a provider that had been swapped out from under it. 28 tests, mostly on READING THE REPLY: kie proxies OpenAI, Google and Anthropic behind one endpoint and they disagree on where the text lives (Anthropic returns content as an ARRAY of parts). Boot verified: [voxel-api] text LLM: kie · gemini-3-7-flash. '
      + '⚠️ NOT VERIFIED, AND THE OWNER SHOULD KNOW WHICH PART: the exact MODEL-ID STRING. kie publishes slugs (gemini-3-7-flash) and has no /v1/models endpoint to confirm they are the API ids, and I will not spend credits guessing. The ENDPOINT is confirmed — probed unauthenticated it returns kie\'s {code:401} envelope while a made-up sibling path returns a plain 404, so routing is real. A wrong slug produces an error that NAMES the model, not a mystery silence. '
      + 'NEXT: one message to the agent on dev.voxel-ai.ai settles it. If it answers, #76 closes and the ENHANCE half becomes a production candidate — that half is NOT Edit Cut and is a live bug today. '
      + '── ✅ WORKING, PROVEN END TO END 2026-08-24 ── Three wrong guesses about the provider before it ran, all three from trusting a cheap signal over the documentation. (1) MODEL ID: I picked gemini-3-7-flash because it was newest — it is the NATIVE Google protocol (/gemini/v1/models/...:streamGenerateContent), not OpenAI-compatible, and would never have worked here. (2) URL: kie puts the model in the PATH, /gemini-2.5-flash/v1/chat/completions. I called the generic /api/v1/chat/completions, which EXISTS and answers "This feature is currently not supported" — reading like a subscription problem and nearly sending the owner to check their kie plan. (3) MY PROBE: I had written "the endpoint IS confirmed" into this card on the strength of an unauthenticated 401. Re-running it later, a model slug I INVENTED returns 401 too — auth is checked before routing on that prefix, so the probe could not fail. It confirmed nothing. Each of the three took one docs fetch to settle. '
      + 'THE PROOF, not a passing unit test: one live call through llm.js with the production AGENT_SYSTEM prompt and a real two-track project. Instruction "delete the castle clip and mute the music". Replied in 2430ms with {"reply":"I have deleted the castle clip and muted the music track for you.", "commands":[{"op":"delete","clipId":"c2"},{"op":"setVolume","clipId":"a1","volume":0}]} — correct JSON, no fence, no preamble, and it resolved BOTH clips by name to the right ids. Cost 0.01 credits. That exact reply is now a fixture in src/lib/edit-agent-live-reply.test.js, which also asserts the EFFECT: the castle clip is really removed and the music really silenced. Every other agent test feeds a reply I wrote myself, which only proves the parser handles what I imagined. '
      + '── A WIDER BUG THIS UNCOVERED, affecting EVERY kie route and predating this work ── The dev log read [EDIT-AGENT] ❌ ??? This feature is currently not supported. That ??? is a status providerErrorParts could not find: it read `status` and `response.status`, while kie.js and llm.js both set `httpStatus`. So every kie failure anywhere in the app logged its status as ???. The log was the smaller half — isProviderRefusal() reads the same value, so a kie 401/403 was never recognised as an account problem and came back to the customer as a generic 500 inviting them to retry forever against something that cannot work until a key or a bill changes. Fixed with 4 tests. '
      + '⚠️ STILL FOR THE OWNER: press the agent in the BROWSER on dev. The server, the provider call, the prompt, the parser and the executor are all proven; what is not is the React panel around them. Lower risk, but it is the half I cannot reach. AND: the ENHANCE-PROMPT fix rides in this same change and is NOT Edit Cut — the red bolt on Image and Video is broken on production right now for exactly this reason. It is a production candidate awaiting the owner\'s go-ahead.' },
  ...
  { ref: '75', owner: 'claude', status: 'done', priority: 2,
    title: 'History is slow for customers with a lot of generations — DONE, the last piece runs itself',
    ...
    detail: 'DONE ON DEV 2026-08-23 (9f81355). CARRIED TO PRODUCTION 2026-08-25 with the owner\'s approval (PR #78) after they reported customers hitting it live — together with TWO MORE CAUSES found on the way: the image grid loaded FULL-SIZE originals eagerly for every fetched row (now loading="lazy" — only cells near the screen download), and every video card used preload="auto", pulling ENTIRE video files just to sit in the grid (now preload="metadata" — the first-frame poster costs kilobytes, hover still plays). '
      ...
      + '── THE LAST PIECE, DONE 2026-09-01 (d7cbe66) ── the small-version builder took an EMAIL: one account per press, forever, so history stayed slow for exactly the customers with the most pictures. The panel put the real number on the screen — 240 presses by hand. A background sweep now does it: 25 every five minutes, newest first, across every account, behind an advisory lock so two production instances cannot pay twice for the same picture. It reuses backfillRows rather than resizing anything itself. '
      + 'MEASURED ON PRODUCTION over 9.7 hours: 234 batches, 5,850 pictures, ZERO failures, 35.3 GB saved, backlog 7,478 → 1,654, and the count falls by exactly 25 per batch 232 times out of 233. Both instances sweep, offset by about 62 seconds; no work is duplicated because each batch skips rows that already have a thumbnail. '
      + 'Two things on the screen were false and live: the card claimed "nothing creates these automatically yet" (untrue since persistWithThumb), and the count included rows the sweep will never take, so it would have stalled above zero for ever and read as a stuck job. Both corrected.' },
  ...
  { ref: '31', owner: 'claude', status: 'in_progress', priority: 1,
    ...
    detail: 'NO LONGER BLOCKED. It was parked on "name the 2–3 behaviours that matter"; the owner answered on 2026-08-22 with the ChatCut screenshots and a written spec, and said it is the whole scope. '
      ...
      + 'CORRECTED 2026-08-31: this card\u2019s \u201cNOT BUILT YET\u201d list was STALE and every item on it is now built AND WIRED \u2014 checked by grepping the render tree, not by reading the card. The real /edit page (signed in on dev goes ProjectPicker to EditCut), the media library that brings in your own generations, the agent chat with a live POST /api/edit-agent, projects stored server-side as the EditProject entity with the server authoritative, regenerate-a-shot-in-place, and captions that render in the viewer AND the export. 580 tests across 26 Edit Cut files. STILL MISSING, verified the same day: \u201cVideo Gen\u201d mode in the composer \u2014 generating a shot from INSIDE the editor rather than leaving for /video, which is the only remaining gap that changes what the product IS and is a decision before it is work \u00b7 a CC on/off toggle (the captions LAYER exists) \u00b7 list view for the media column \u00b7 ChatCut\u2019s agent furniture (Voices, Design Style, Skills, Thinking Mode, Motion Graphics Quality) which we may want none of \u00b7 collaborators and report-a-bug. IT IS SWITCHED OFF ON PRODUCTION BY DESIGN \u2014 VITE_EDIT_CUT is unset, so a signed-in customer on voxel-ai.ai sees the waitlist. Dev and localhost get the editor. '
  ...
  { ref: '36', owner: 'claude', status: 'done', priority: 13,
    ...
    detail: 'DONE 2026-09-01. Top of the SOP tab, above the zones — a check you have to scroll to is a check that happens after the room has sat down. Type the workshop code, press Check. '
      + '── THE FOUR ── alerts green? · supplier balance with DAYS OF RUNWAY (and the sentence that a workshop is not an average day) · has any model gone bad? · does this cohort’s access work today? '
      + '── IT LEADS WITH A SENTENCE, NOT A DASHBOARD ── "Ready to start." or "Do not start yet — supplier balance", with the reason beside it. A dashboard makes you decide; ten minutes before a workshop the useful output is start or do not start. '
      + '── NOTHING HERE IS NEW DATA ── every fact is borrowed from the tab that already owned it: the alert rows from the same query Alerts runs, the balance from the same gatherFacts() the SOP zones use, the model verdicts from gatherReliability() behind Costing → Reliability (extracted from that route so there is ONE copy of the attribution query, not two that can disagree about which model is safe to teach). So this screen can never contradict the tab it came from. '
      + '── THE ONE GENUINELY NEW CHECK ── credits die 30 days after they are granted, so a cohort that redeemed at an earlier session has a perfectly valid code and nothing to spend. The code is green and the attendee has zero. Nothing else put those two facts on one screen. '
      + '── TWO BUGS THE TESTS CAUGHT, BOTH THE 8 AUGUST SHAPE ── (1) worst() inherited the SOP engine\'s ranking where WARN beats UNKNOWN, so an unreadable balance beside any minor warning produced "Safe to start" — go: true, over exactly the check that was missing on 8 August. UNKNOWN now outranks WARN on this screen only, and the SOP tab keeps its own ordering on purpose. (2) a failed credit-expiry read passed null, and null > 0 is false, so it fell through to the green path and called the cohort healthy on the strength of not having looked. That default now fails closed. '
      + '── WHAT IT DELIBERATELY DOES NOT CHECK ── FAL. They publish no balance endpoint, which is why an empty FAL account is only ever visible afterwards as a wave of refunds. The one thing that CAN be probed without a key is fal.ai\'s public model list — and that is exactly why it is not here: it would answer "is fal.ai\'s website up", print a green light, and be read as "FAL will work today". '
      + '47 tests. Verified by unmounting the panel, by moving it below the zones, and by reinstating each of the two bugs above — each sabotage fails the suite.' },
  ...
  { ref: '72', owner: 'claude', status: 'pending', priority: 14,
    ...
    detail: '── DIAGNOSED 2026-08-23 FROM THE OWNER’S PHONE SCREENSHOT ── The Camera Settings sheet on /Image lays CAMERAS · LENSES · FOCAL side by side in THREE columns. On a phone the third is cut off at the right edge: 14mm and 18mm are sliced in half, there is no horizontal scroll, and THE FOCAL LENGTH CANNOT BE CHOSEN AT ALL. Component: src/components/image/CameraSelector.jsx. Not a cosmetic squeeze — one of the three choices is unreachable, and camera metadata is a distinguishing feature of the platform, so it fails on the device most people carry. Fix by stacking or scrolling, NOT by shrinking the type, which trades unreachable for unreadable. '
      ...
      + '── SWEEP DONE 2026-08-24 · 390 / 768 / 1024 / 1194 / 1280 · ALL 12 ROUTES · findings in docs/RESPONSIVE-SWEEP.md ── Measured in a real browser by comparing every element against the viewport, not by reading breakpoints out of the CSS. '
      + '⚠️ THE BIGGEST FINDING WAS NOT ON A PHONE, AND IT VINDICATES THE OWNER WIDENING THIS CARD. At 390px and 768px every page was clean. At 1024px EVERY PAGE was broken: the nav was `hidden lg:flex` and the hamburger `lg:hidden`, and Tailwind\'s lg: is 1024px — so the desktop header switched ON and the fallback switched OFF at exactly the width where the desktop header stops fitting. Measured: the nav ended at x=1040 and the Login / Sign Up group ran x=1040→1218. Not clipped-but-scrollable — GONE, with no menu button. iPad 1080, iPad Air 1180, iPad Pro 11-inch 1194 all sit in that band. A visitor on an iPad in landscape COULD NOT SIGN IN OR SIGN UP. Fixed: the header now swaps at xl: (1280), which is safe only because the hamburger panel already carries the auth buttons — a test asserts it still does. '
      + 'FIXED AND VERIFIED: the Camera Settings sheet (the original report — four columns measured 128/67/52/58px at 390px, now 2×2 with every cell 158px); /Image prompt bar wrapping; /Video split column; /Audio panels 76px past the edge (the page already collapsed to one column, but a grid item defaults to min-width:auto so the single 1fr track floored at 438px inside a 334px container — going to one column does nothing if the one column cannot shrink); Explore\'s 7×7px carousel dots; the 18×18px image-count stepper, which also had no tooltip at all. '
      + 'GUARDED, per the standing rule that a review should end in a test and not a memory: src/components/navigation/Navbar.breakpoint.test.jsx — the four classes that swap nav and hamburger must agree on ONE breakpoint, it must be at or above the measured 1240px requirement, and the mobile panel must keep Login and Sign Up. '
      + '── STILL OPEN, WHICH IS WHY THIS CARD STAYS OPEN ── (a) THE CONTROL PANEL is unswept: it is behind an admin login and I do not sign in, so the owner has to walk it or hand me screenshots. (b) THE SUPPORT BUBBLE has nowhere safe on a phone — measured, it steals clicks from GENERATE in the bottom-right and from the Motion Control tab in the top-right. It is currently top-right because blocking one tab of three beats blocking the primary action, but neither is right and the real answer (move it into the header on small screens) is a change to the site chrome that is the owner\'s call. (c) TOUCH TARGETS in the 26–30px band on /Video — hittable, but under Apple\'s 44. (d) 1440px and above. '
      + '── AND A WARNING FOR WHOEVER SWEEPS NEXT, INCLUDING ME ── The false positives OUTNUMBERED the real findings, three to one. The first pass reported 1646px of overflow on /Explore — a working carousel. Correcting for that, I then treated overflow-x:hidden as "fine, it scrolls", which is backwards and was exactly what hid the /Audio bug. And 700/900px unclassed divs on three pages turned out to be blurred decorative glows positioned to bleed off-frame on purpose — I was one step from "fixing" the page background. A sweep tool that cries wolf produces work instead of finding it.' },
  ...
  { ref: '71', owner: 'claude', status: 'done', priority: 20,
    ...
    detail: 'CAUSE — index.js:7068: `setTimeout(runAutomatedBackup, 5 * 60 * 1000)` fires five minutes after EVERY BOOT, with no "has today already been written?" guard. Production runs TWO instances, so ONE deploy produces TWO backups; sixteen versions is eight deploys. '
      ...
      + '── ✅ CONFIRMED HELD 2026-08-24, from the RUNTIME LOGS, not a screenshot ── The dated check on this card was "look in Backblaze, today must show no version count", which needed the owner because our code cannot LIST that bucket (#70). It did not need them: DigitalOcean\'s runtime log answers it directly and more precisely. '
  ...
  { ref: '96', owner: 'claude', status: 'done', priority: 1,
    title: 'TEN people could not redeem the code they were invited to',
    why: 'Happened live on 2026-09-02 during the SPA New Academy session. osama.himselff@gmail.com tried the code TWELVE times in twenty minutes and was refused every time. At least three others complained the same day.',
    detail: 'THE INVISIBLE CHARACTER WAS THE SMALLER HALF. His address is stored as "\u200fosama.himselff@gmail.com" \u2014 a RIGHT-TO-LEFT MARK, inserted silently by Arabic-language Excel (the sheet is titled \u0628\u064a\u0627\u0646\u0627\u062a \u0627\u0644\u0637\u0644\u0627\u0628). Invisible in Excel, in the invites drawer and in an email; only the bytes differ. '
      + '\u2014\u2014 BUT CHASING IT FOUND THE REAL FAULT. mayRedeem() normalised the address as TYPED and trusted the address as STORED: given a Set, it assumed the caller had already cleaned it, and the redeem route passes a Set built straight from the database. So "ahmed@gmail.com" was compared against "Ahmed@Gmail.com" and refused. NINE of the 84 have capitals. Ten people, not one \u2014 and the capitals had been failing quietly since invite lists shipped in August, every failure looking to the customer like "you were not invited". '
      + 'MY EARLIER NOTE ON THIS ROW SAID THE 83 OTHERS WERE CLEAN BECAUSE CAPITALS ARE "LOWERCASED ALREADY". That was wrong, and it is why the row is worth reading twice: the reasoning was right about the function and never checked the call site. '
      + '\u2014\u2014 FIXED, three parts \u2014\u2014 (1) BOTH SIDES normalised at comparison, which repairs every list ALREADY in the database with no re-upload and nobody\u2019s row edited \u2014 that is what unstuck the people who were stuck. (2) new imports stored clean, so the drawer shows an address a human can read. (3) the SEAT is now ticked off by ROW, using the same comparison that admitted the person; the old SQL did LOWER(email)=LOWER($3), which agrees about capitals and DISAGREES about an invisible mark \u2014 so the code would have shown fully redeemed with every seat still empty. '
      ...
      + '\u2014\u2014 VERIFIED ON PRODUCTION 2026-09-02 \u2014\u2014 the new SOP line "Accounts nobody can sign in to" reads 0, green, over every account. So nothing was ALREADY broken by a stored address, and — the thing worth recording — the one regression this fix could itself have caused did NOT happen: normalising the typed address at sign-in would have locked out anyone whose STORED address carried an invisible mark, and there are none. 36 tests, three sabotages.' },
  ...
  { ref: '84', owner: 'claude', status: 'done', priority: 1,
    ...
    detail: 'Soft delete, a confirmation naming the FILTER as well as the count, an undo, the customer\u2019s own "Recently deleted", and a purge that makes "permanently deleted after 30 days" actually true. Purge removes the ROW before the FILE, so the safe failure is an orphan rather than a row that looks recoverable and is not. Proven by Amr on dev: delete, reload, restore, back in history.' },
  { ref: '85', owner: 'claude', status: 'done', priority: 2,
    title: 'Search your own history — there was none at all',
    why: 'A customer with 349 pictures looking for last Tuesday had exactly one option: scroll. The grid was already paged and lazy, so the remaining pain was never loading — it was FINDING.',
    detail: 'Words, date, model, with a COUNT so "too narrow" is distinguishable from "not there". Nothing is narrowed by default: a promo code is used across a MONTH, so presets are 30/60 days. The grid also loads two pages AHEAD of the scroll. Model list built from what each customer has actually used.' },
  { ref: '86', owner: 'claude', status: 'done', priority: 3,
    title: 'Recovery tab — put back a customer\u2019s work when they cannot',
    why: 'Their own screen handles the ordinary mistake. A closed account, a bulk mistake, or somebody who cannot find it needs you.',
    detail: 'Account / words / when, sorted by TIME REMAINING rather than deletion date. Restore says how many of how many — asking for 40 and getting 38 is a fact a support call needs. Refuses anything past 30 days: the file may be gone and "restored" would be a lie.' },
  { ref: '87', owner: 'claude', status: 'done', priority: 4,
    title: 'Six customers were refunded for images that had actually SUCCEEDED',
    why: 'Nano Banana Pro: 6 of 19 attempts in one afternoon were refunded as "taking too long". All six finished — at 94, 97, 125, 130, 144 and 314 seconds. We paid the provider for every one and the customer got nothing.',
    detail: 'The 90s wait is not arbitrary (the edge cuts a request at ~100s), so the REQUEST ends and the JOB continues: recorded before the response goes out, finished by the browser or by a sweeper for the tab that never came back. Exactly-once via a conditional UPDATE, safe across two instances and two tabs.' },
  { ref: '88', owner: 'claude', status: 'done', priority: 5,
    title: 'Every new picture gets its small version automatically',
    why: 'The grid READ thumb_url, the backfill button WROTE it, and nothing created one at generation time. So every backfill was temporary and the grid got slower again every day — and a control-panel card claimed it was automatic when it was not.',
    detail: 'Made from the same download that re-hosts the picture. A missing thumbnail costs one slow grid cell; an exception would cost the customer their picture — so every failure returns "no thumbnail" and hands the image over unchanged. PROVEN on production: 119 pictures generated, 119 thumbnails, zero failures.' },
  { ref: '89', owner: 'claude', status: 'done', priority: 6,
    title: 'The backup no longer stops when Backblaze cannot be listed',
    why: 'Every 15 minutes the sync listed the entire offsite bucket to work out what was missing. That one operation has been unreliable since 20 August, and on the 29th it stopped the backup for SEVENTEEN HOURS while customers generated all day.',
    detail: 'Our own database records what has been copied (offsite_media); the listing is demoted to a seeder, so it ships without re-uploading 72 GB. Nothing is recorded until the bytes are read back at the right size — a ledger that remembers a copy which did not happen would skip that file forever, silently. Copying twice beats not copying at all.' },
  { ref: '90', owner: 'claude', status: 'done', priority: 7,
    title: 'The backup now TELLS you when it stops',
    why: 'It stopped for 17 hours while the alerts pass logged "0 open" every five minutes. There were alerts for the restore check going stale, for the provider balance, for stuck charges — and none for the backup itself having stopped. It was found by accident.',
    detail: 'A heartbeat row written only on a clean pass — a variable would reset to healthy on every deploy and never fire. Warning at 90 minutes, critical at six hours. Never reassuring when it does not know: no record, a broken timestamp or a future date all raise. Amr saw it fire on production the same night.' },
  { ref: '91', owner: 'owner', status: 'pending', priority: 8,
    title: 'Stop keeping deleted files forever — 60-day version expiry',
    why: 'Versioning is on, which is what makes a stolen key survivable. It also means a deleted file is never really deleted, so "permanently deleted after 30 days" is true of the service and not of the storage. It matters for the B2B legal documents.',
    detail: 'BUILT AND APPLIED TO NOTHING. Control panel → SOP → Jobs you run by hand → "Stop keeping deleted files forever". Press Preview first: it writes nothing and says exactly what would change. Not urgent — do it when finalising the legal documents.' },
  { ref: '83', owner: 'claude', status: 'done', priority: 1,
    title: 'RESCUE the customer files still on a provider link — now running on its own',
