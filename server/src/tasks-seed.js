...
export const SEED = [
  ...
  { ref: '101', owner: 'claude', status: 'in_progress', priority: 1,
    ...
    detail: '── WHAT WAS VERIFIED FIRST ── Voxel\'s request for the model named Kling 3.0 was already correct: Multi Shot toggle defaults OFF, the page passes it, the server sends multi_shots:false explicitly with the image in image_urls — and that is exactly the single-shot form kie\'s own Kling 3.0 docs specify (their default is false too; real multi-shot needs a multi_prompt array we never send). So the flag was never the bug. '
      + '── THE SUSPECTS A CORRECT FLAG CANNOT STOP ── (1) Kling itself "storyboarding" a prompt that reads like several scenes, even in single-shot mode. (2) The customer being on Kling 3.0 OMNI — nearly identically named in the picker — which still runs on the legacy FAL path with no shot control sent at all. (3) A kie-side regression. (Correction to what was said aloud: the second routing table that "disagrees" is VIDEO_MODELS, a legacy history-id lookup, not the router — no routing bug there.) '
      + '── FIXED 2026-08-25, ON THE BRANCH, COVERING ALL THREE ── every Kling-family image-to-video request with Multi Shot OFF now carries a continuity instruction in the PROVIDER-facing prompt ("Single continuous shot animated from the provided image — no cuts, no scene changes, no camera switches"); the customer\'s own prompt is stored and shown untouched, exactly like the camera-motion merge. The FAL Kling v3 path (Omni) additionally sends shot_type:"customize" explicitly for image-to-video — the documented "shots are what the prompt says" mode — so no provider default can storyboard a single image. Pure helper with 8 tests; wiring pinned. '
      + '── THE PROPER NEXT STEP ── move Kling 3.0 Omni off the legacy FAL path onto kie\'s Kling O3, whose schema exposes multi_shots, prefer_multi_shots AND customize_multi_shots (all to be sent false for image-to-video) plus video references. Blocked on the same two facts as Omni 1.1: kie\'s exact O3 model id and KIE\'S OWN price line, read from their page by the owner — aggregators quote figures that must not be trusted for pricing. '
      + '── STILL FOR THE OWNER ── one real Kling 3.0 image-to-video on dev after deploy: the [VIDEO] continuity guard applied line appears in the log, the payload shows multi_shots:false, and the clip stays on the image. If a cut-up clip STILL comes back with that payload, it is kie-side and the taskId + payload line is the support ticket.' },
