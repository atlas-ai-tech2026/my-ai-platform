    ...
    <div>
      <Card step="1" title="What kind of message?" why="Manual notifications you compose. Automatic ones live in the Automations tab.">
        ...
        <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', marginTop: 14 }}>
          <Field label="Title">
            <input value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="e.g. Eid offer: 15% off everything" style={{ ...input, width: 320 }} />
          </Field>
        </div>
        <div style={{ marginTop: 10 }}>
          <Field label="Message — variables: {name} {plan} {credits}">
            <textarea value={body} onChange={(e) => setBody(e.target.value)}
              placeholder="Write the message your clients will see…"
              style={{ ...input, width: '100%', maxWidth: 620, height: 76, padding: 10, resize: 'vertical' }} />
          </Field>
        </div>
        ...
        <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', marginTop: 12, alignItems: 'flex-end' }}>
          ...
          <Field label="Button link (a path on this site)">
            <input value={ctaUrl} onChange={(e) => setCtaUrl(e.target.value)}
              placeholder="/pricing" style={{ ...input, width: 220 }} />
          </Field>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {errs.length > 0 && (
          <span style={{ color: '#f87171', fontWeight: 600, fontSize: 12.5 }}>Fix first: {errs.join(' · ')}</span>
        )}
      </div>
      <div style={{ ...muted, marginTop: 10, fontSize: 12 }}>Sending writes the audit log.</div>
    </div>
  );
}
